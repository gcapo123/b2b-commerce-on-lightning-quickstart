({
        ...
        action.setCallback(this,function(response){
            ...
            if (state === "SUCCESS") {
                ...
                for (var i = 0; i < rows.length; i++){
                    ...
                    if (row.Project_Milestone__c) row.Phase = row.Project_Milestone__r.Name;
                    
                    //Unique phase list
                    if(!phaseSet.has(row.Project_Milestone__r.Id)){
                        phaseSet.add(row.Project_Milestone__r.Id);    
                        phaseList.push({'label':row.Project_Milestone__r.Name, 'value':row.Project_Milestone__r.Id});
                    }
    ...
    cloneTask: function(component, taskRec) {
        if(taskRec) {
            ...
            createPMTTaskEvent.setParams({
                ...
                "defaultFieldValues": {
                    'Project_Milestone__c' : taskRec.Project_Milestone__c,
                    'Name' : taskRec.Name,
    ...
    updateTable: function (component) {
        ...
        if(selectedPhase !== 'all' && selectedPhase!=='') {
            filteredRows = filteredRows.filter(row=> row.Project_Milestone__r.Id == selectedPhase);
        }
